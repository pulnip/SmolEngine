...
void BowComponent::EndAim(Smol::Vec3 direction){
    //TODO. 활 조준 종료
    ...
    //TODO. 화살이 이동할 궤적
    std::vector<Smol::Vec3> arrowTrajectory = BuildTrajectory(direction);

    // 계산한 궤적을 화살에게 넘겨서 샘플 위치를 따라 이동시킨다.
    arrow->SetTrajectory(std::move(arrowTrajectory), 0.1f);
}

std::vector<Smol::Vec3> BowComponent::BuildTrajectory(Smol::Vec3 direction)
{
    // 임시 변수
    ...
    for (int i = 0; i < sampleCount; ++i)
    {
        ...
        // 위치 = 시작 위치 + 초기 속도 * 시간 + 1/2 * 가속도 * 시간^2
        Smol::Vec3 pos = startPos + arrowVelocity * t + 0.5f * gravity * t * t;
